...
//        return contents;
    ...
    // 실험2. 첨부로 포함 찾기
    public List<Content> findContentsByAttachFilePlus(String attachFile){

        return contentRepository.findAllByAttachFileLike(attachFile);

    }

    // 실험3. 조건 2개로 찾기
    public List<Content> findContentsByTwo(String title, String attachFile){

        return contentRepository.findAllByTitleAndAttachFile(title, attachFile);
    ...
    // 실험4-2. 쿼리로 값하나 반납
    public String findContentsBySpeci2(long noticeId){

        Content findAttachFile = contentRepository.findAttachFileByNoticeId2(noticeId);
        String result = findAttachFile.getAttachFile();
